...
import data_augmentation_method as DA
import json
...
def transform(image, aug_):
    segmentation_map = np.zeros((480, 640, 3))
    segmentation_map = np.where(image > 0, 255, 0)
    ...
    #cv2.rectangle(segmentation_map, (x, y), (x + w, y + h), (255, 255, 255), 2)
    x_scales = [150, 200]
    y_scales = [150, 200]
    transformed_image_list = [[]]
    for x_scale in x_scales:
        for y_scale in y_scales:
            out = aug_.scale_image(image_cut, x_scale, y_scale)
            # plt.imshow(out)
            # plt.show()
            out_shape_ = out.shape
            transformed_image = np.zeros((480, 640, 3)).astype(np.uint8)
            transformed_image_shape_ = transformed_image.shape
            x_s = transformed_image_shape_[0]-out_shape_[0]-4
            y_s = transformed_image_shape_[1]-out_shape_[1]-4
            if x_s > 0 and y_s > 0:
                x_l = random.randint(0, x_s)
                y_l = random.randint(0, y_s)
                transformed_image[x_l:x_l+out_shape_[0], y_l:y_l+out_shape_[1], :] = out[:, :, :]
...
def data_engin(image_path, packages, background_path, aug_background, aug_package, image_id, object_id ,coco_data):
    ...
    for function_ in selected_names:
        # print(function_)
        back_img = getattr(aug, function_)(back_img)
        back_img_save = back_img[:]

    back_img = cv2.resize(back_img, (640, 480))
    back_img_save = cv2.resize(back_img, (640, 480))
    image_input_list = [[]]
    while len(image_input_list) < number_packages_in_scene:
        while True:
            ...
            start_range_img = 0;
            end_range_img = len(picked_img) - 1
            ...
            process_mask = np.where(image_input > 0, 1, 0)
            process_mask =process_mask[:, :, 0]
            my_area = sum(sum(process_mask))
            if my_area >= 4900:
                break
        ans = transform(image_input, aug_)
        image_input_list.extend(ans[1:])
    for i in range(1, len(image_input_list)):
        back_img = np.where(image_input_list[i] == 0, back_img, image_input_list[i])
        back_img_save = np.where(image_input_list[i] == 0, back_img_save, image_input_list[i])
        segmentation_map = np.where(image_input_list[i] > 0, i, segmentation_map)
